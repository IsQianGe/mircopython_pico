...
typedef struct _esp8285_nic_obj_t
{
    mp_obj_base_t base;

    int8_t sock_id;    // sock id generate by esp8285, user can't see this id
...
/* STATIC mp_obj_t esp8285_nic_ifconfig(mp_obj_t self_in) {
    esp8285_spi_net_t* inet = esp8285_spi_get_network_data();
	if(inet == NULL)
	{
...
STATIC mp_obj_t esp8285_nic_isconnected(mp_obj_t self_in) {
    uint8_t is_connected = esp8285_spi_is_connected();
    return mp_obj_new_bool(is_connected == 0);
}

STATIC mp_obj_t esp8285_nic_disconnect(mp_obj_t self_in) {
    esp8285_spi_disconnect_from_AP();
    return mp_const_none;
}

STATIC mp_obj_t esp8285_nic_ping(size_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
	enum { ARG_host, ARG_host_type};
	esp8285_nic_obj_t* self = NULL;
    ...
    int32_t time = esp8285_spi_ping((uint8_t*)host, host_type, 100);
    if(time == -2)
    {
...
STATIC mp_obj_t esp8285_nic_connect( size_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args )
{
    ...
    int8_t err = esp8285_spi_connect_AP((uint8_t*)ssid, (uint8_t*)key, 20);
    if(err != 0)
    {
...
STATIC mp_obj_t esp8285_scan_wifi( mp_obj_t self_in )
{
    ...
    esp8285_spi_aps_list_t* aps_list = esp8285_spi_scan_networks();
    if(aps_list == NULL)
        goto err;
    uint32_t count = aps_list->aps_num;
    for(int i=0; i<count; i++)
    {
        esp8285_spi_ap_t* ap = aps_list->aps[i];
        mp_obj_t info[3];
...
STATIC void esp8285_make_new_helper(esp8285_nic_obj_t *self, size_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args)
{
    ...
    static const mp_arg_t allowed_args[] = {
        ...
        {MP_QSTR_rst, MP_ARG_KW_ONLY | MP_ARG_OBJ, {.u_rom_obj = MP_ROM_NONE} },
        {MP_QSTR_rdy, MP_ARG_KW_ONLY | MP_ARG_OBJ, {.u_rom_obj = MP_ROM_NONE} },
    ...
    esp8285_spi_config_io(cs , rst, rdy ,
                        mosi , miso , sclk );
    esp8285_spi_init();
    /* char* version = m_new(char, 32);
    char* ret = esp8285_spi_firmware_version(version);
    if(ret == NULL)
    {
...
STATIC mp_obj_t esp8285_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *args)
{
    ...
    esp8285_nic_obj_t *self = m_new_obj(esp8285_nic_obj_t);
    self->base.type = (mp_obj_type_t *)&mod_network_nic_type_esp8285;
    self->sock_id = -1;
    self->firmware_ver = NULL;
    mp_map_t kw_args;
    mp_map_init_fixed_table(&kw_args, n_kw, args + n_args);
    esp8285_make_new_helper(self, n_args, args, &kw_args);
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/* 
STATIC mp_obj_t esp8285_firmware_version(mp_obj_t self_in)
{
    esp8285_nic_obj_t* self = (esp8285_nic_obj_t*)self_in;
...
/* STATIC mp_obj_t esp8285_adc(size_t n_args, const mp_obj_t *pos_args) {
    ...
    if (esp8285_spi_get_adc_val(channels, len, adc) == 0)
    {
        mp_obj_t *tuple, *tmp;
    ...
    {
        mp_raise_ValueError("[MaixPy]: esp8285 read adc failed!\r\n");
    }
} */

/* STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(esp8285_adc_obj, 1, 2, esp8285_adc);
STATIC MP_DEFINE_CONST_FUN_OBJ_1(esp8285_version_obj, esp8285_firmware_version); */
...
STATIC MP_DEFINE_CONST_FUN_OBJ_KW(esp8285_nic_ping_obj, 1, esp8285_nic_ping);
...
/*     {MP_ROM_QSTR(MP_QSTR_version), MP_ROM_PTR(&esp8285_version_obj)},
    {MP_ROM_QSTR(MP_QSTR_adc), MP_ROM_PTR(&esp8285_adc_obj)}, */
...
/*    { MP_ROM_QSTR(MP_QSTR_connect), MP_ROM_PTR(&esp8285_nic_connect_obj) },
    ...
    { MP_ROM_QSTR(MP_QSTR_ping), MP_ROM_PTR(&esp8285_nic_ping_obj) },
...
STATIC int esp8285_socket_socket(mod_network_socket_obj_t *socket, int *_errno) {
    esp8285_nic_obj_t* nic = (esp8285_nic_obj_t*)socket->nic;
...
STATIC int esp8285_socket_connect(mod_network_socket_obj_t *socket, byte *ip, mp_uint_t port, int *_errno) {
	...
	switch(socket->u_param.type)
	{
		...
		{
            uint8_t ret = esp8285_spi_get_socket();
            if(ret == 0xff)
            {
                *_errno = MP_EIO;
                return -1;
            ...
            int8_t conn = esp8285_spi_socket_connect((uint8_t)nic->sock_id, ip, 0, port, TCP_MODE);
			if(-2 == conn)
			{
				*_errno = MP_EIO;
				return -1;
...
STATIC mp_uint_t esp8285_socket_recv(mod_network_socket_obj_t *socket, byte *buf, mp_uint_t len, int *_errno) {
    ...
    esp8285_nic_obj_t* self = (esp8285_nic_obj_t*)socket->nic;
    int read_len = 0;
    ...
    do{
        if(self->sock_id >= 0)
        {
            len_avail = esp8285_spi_socket_available(self->sock_id);
            if(len_avail == -1)
            {
                *_errno = MP_EIO;
                return MP_STREAM_ERROR;
            ...
            if(len_avail > 0)
            {
                ...
                ret = esp8285_spi_socket_read(self->sock_id, (uint8_t*)buf+read_len, once_read_len);
                if(ret == -1)
                {
                    *_errno = MP_EIO;
                    return MP_STREAM_ERROR;
        ...
        {
            if((self->sock_id < 0) || (esp8285_spi_socket_status(self->sock_id) == SOCKET_CLOSED))
            {
                self->sock_id = -1;
...
STATIC mp_uint_t esp8285_socket_send(mod_network_socket_obj_t *socket, const byte *buf, mp_uint_t len, int *_errno) {
    ...
    esp8285_nic_obj_t* self = (esp8285_nic_obj_t*)socket->nic;
    if(self->sock_id >= 0)
        status = esp8285_spi_socket_status(self->sock_id);
    ...
    while(1)
    {
        ...
        //TODO: esp8285_spi_socket_write is nonblock in esp8285 firmware
        if(esp8285_spi_socket_write(self->sock_id, (uint8_t*)buf, len_send ) == 0)
        {
            *_errno = MP_EIO;
            return -1;
        }
 ...
 */
/* STATIC mp_uint_t esp8285_socket_sendto(mod_network_socket_obj_t *socket, const byte *buf, mp_uint_t len,  uint8_t* ip, mp_uint_t port, int *_errno) 
{
    int8_t ret;
	if((mp_obj_type_t*)&mod_network_nic_type_esp8285 != mp_obj_get_type(MP_OBJ_TO_PTR(socket->nic)))
	{
    ...
    self->to_be_closed = false;
    ret = esp8285_spi_socket_connect((uint8_t)self->sock_id, ip, 0, port, UDP_MODE);
    if(-2 == ret)
    {
        *_errno = MP_EIO;
        return -1;
    ...
    ret = esp8285_spi_add_udp_data((uint8_t)self->sock_id, (uint8_t*)buf, (uint16_t)len);
    if(ret != 0)
    {
        *_errno = MP_EIO;
        return -1;
    }
    ret = esp8285_spi_send_udp_data((uint8_t)self->sock_id);
    if(ret !=0 )
    {
        *_errno = MP_EIO;
        return -1;
...
STATIC mp_uint_t esp8285_socket_recvfrom(mod_network_socket_obj_t *socket, byte *buf, mp_uint_t len,  byte* ip, mp_uint_t* port, int *_errno)
{
   if((mp_obj_type_t*)&mod_network_nic_type_esp8285 != mp_obj_get_type(MP_OBJ_TO_PTR(socket->nic)))
	{
		*_errno = MP_EPIPE;
    ...
    int8_t res = -1;
    if( self->sock_id >= 0)
        res = esp8285_spi_get_remote_info(self->sock_id, (uint8_t*)ip, &port_16);
    if(res !=0)
    {
        *_errno = MP_EIO;
        return MP_STREAM_ERROR;
...
STATIC void esp8285_socket_close(mod_network_socket_obj_t *socket) {
    ...
    if(self->socket_fd == socket->fd)
    {
        if(self->sock_id >= 0)
        {
            esp8285_spi_socket_close((uint8_t)self->sock_id);
            self->sock_id = -1;
        }
    }
}
...
/* STATIC int esp8285_socket_gethostbyname(mp_obj_t nic, const char *name, mp_uint_t len, uint8_t* out_ip) {
    ...
    uint8_t tmp_ip[6];
    int ret = esp8285_spi_get_host_by_name((uint8_t*)name, tmp_ip);
    if( ret != 0)
        return ret;
...
/*     .gethostbyname = esp8285_socket_gethostbyname,
    ...
    .sendto = esp8285_socket_sendto,
    .recvfrom = esp8285_socket_recvfrom, */
    /*
